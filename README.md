...
> **Дружит с root.** Без root Wy Store работает целиком, только каждую установку подтверждает
> Android. С root он делает всё сам: скачивает обновление в фоне и ставит его через `pm install`, не
> показывая ни одного диалога — с той же проверкой подписи, что и всегда. Оба переключателя лежат в
> настройках выключенными, пока вы их не включите и root действительно не выдан.
...
- Root не обязателен: без него доступно всё, кроме тихой установки без диалогов
- Компактные экраны: вёрстка проверена на Asus Zenfone 10 и на большом телефоне
