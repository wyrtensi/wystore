...
> **Обновляется молча и без root.** Android разрешает обновлять приложение без диалога тому, кто
> его установил. Wy Store этим пользуется: то, что он поставил сам, он и обновляет сам — скачивает,
> проверяет подпись и ставит, ничего не спрашивая, даже когда приложение свёрнуто. Первая установка
> подтверждается всегда, и чужое приложение тоже — пока вы не передадите его обновления Wy Store с
> его же карточки. С root тихой становится и первая установка.

> **Android 8.0 — минимум самого Wy Store, а не каталога.** Многим приложениям из каталогов нужен
> Android новее: 9, 10, а иногда 12 или 13. Требуемая версия написана на странице каждого
...
**Обновления, которые доходят сами.** Из коробки цепочка замыкается целиком: проверка находит
обновление, сразу качает его и ставит. Никакого root для этого не нужно — Android с двенадцатой
версии разрешает установщику обновлять то, что он сам установил, без подтверждения. Приложение при
этом может быть свёрнуто: коммит сессии установки не требует, чтобы Wy Store был на экране.

- *Скачивать обновления сразу* — найденное начинает качаться само, включено по умолчанию. Ручная
  проверка качает немедленно, потому что вы стоите и ждёте ответа. Фоновая уважает настройки сети:
  при «только Wi-Fi» дождётся Wi-Fi, а не потратит мобильный трафик.
- *Устанавливать сразу после скачивания* — обновления и новые приложения, оба включены по умолчанию.
- *Обновлять без вопросов* — пропускать системный диалог там, где Android это позволяет.

Границы у этого честные. Первая установка спрашивает всегда — Android не даёт её обойти никому.
Приложение, которое поставил другой магазин, тоже спрашивает: обновлять молча вправе только тот, кто
установил. Передать это право Wy Store можно кнопкой на карточке приложения — она переустановит его
через Wy Store, после чего обновления пойдут тихо.

С root тихой становится и первая установка: скачанное ставится через `pm install`, без диалога.
Переключатель лежит в настройках выключенным, пока вы его не включите и root действительно не выдан.

Проверки при этом те же самые: имя пакета, versionCode, полнота комплекта APK и совпадение подписи.
Тихая установка не ослабляет ни одну из них — APK с чужой подписью будет отклонён и здесь.
...
- Первая установка приложения всегда проходит через системный диалог Android — обойти его нельзя.
- Обновление проходит без диалога только там, где это разрешает сама система: приложение уже
  установлено, и установил его Wy Store. Отключается одним переключателем в настройках.
...
- Android 8.0 (API 26) или новее — это требование самого Wy Store; приложениям из каталогов
  часто нужна версия новее, и она указана на странице каждого из них
- Разрешение на установку неизвестных приложений — Android запросит его при первой установке
- Root не обязателен: без него доступно всё, кроме тихой **первой** установки
- Компактные экраны: вёрстка проверена на Asus Zenfone 10 и на обычных экранах
