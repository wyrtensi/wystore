...
- **Android TV.** Той самий APK працює на Android TV і Google TV, з власним інтерфейсом під пульт
  і каталогом RuStore для телевізорів. Подробиці — у розділі [Android TV](#android-tv).
- **Root за бажанням.** З наданим root перше встановлення, оновлення та видалення проходять без
  вікон Android, зокрема у фоні. За замовчуванням вимкнено; подробиці — у розділі [Root](#root).
- **Інтерфейс.** Русский, English, Українська, Беларуская, Қазақша, Oʻzbekcha, Latviešu, 简体中文.
  Світла й темна тема, Material You.

## Android TV

| Головна | Сторінка застосунку | Мої застосунки |
|---|---|---|
| ![Головна на телевізорі](docs/screenshots/tv/home.png) | ![Сторінка застосунку на телевізорі](docs/screenshots/tv/app-page.png) | ![Мої застосунки на телевізорі](docs/screenshots/tv/my-apps.png) |

Окремої збірки немає: той самий APK ставиться на Android TV і Google TV з Android 9 і новіших та
з'являється в лаунчері телевізора. Тип пристрою визначається сам; якщо приставка визначилася як
телефон, його можна вибрати вручну в **Налаштування → Зовнішній вигляд → Тип пристрою** (авто,
телефон, телевізор).

- **Керування з пульта.** Меню вкладками вгорі — Головна, Пошук, Мої застосунки, Налаштування;
  поруч із назвою Wy Store стоїть позначка «ТБ». Кнопка «Назад» веде назовні: із вмісту в меню,
  з меню на Головну, з Головної — вихід. Після повернення фокус стоїть на тій картці, яку
  відкривали.
- **Головна.** Картка оновлень, плитки розділів, ряди за розділами і ряд «З GitHub». Сповіщень на
  телевізорі немає, тому застосунки, готові до встановлення або оновлення, зібрано в ряд угорі
  Головної.
- **Сторінка застосунку.** Скриншоти для телевізора та відгуки.
- **Пошук.** Текстом або голосом. Результати — з ТБ-каталогу, а якщо увімкнено каталог для
  телефона, то й з усього RuStore, плюс GitHub.
- **Каталог.** У RuStore окремий каталог для телевізорів. Налаштування **«Каталог на телевізорі»**:
  «Для телевізора» (за замовчуванням), «Для телефона» або «Обидва». У режимі «Обидва» застосунки
  для телефона йдуть своїми рядами і позначені «Для телефона»: багато з них не розраховані на
  пульт. Застосунки, встановлені з будь-якого каталогу, і далі оновлюються за будь-якого вибору.
- **GitHub.** Каталог GitHub є і на телефоні, і на телевізорі; вимикається в
  **Налаштування → Джерела застосунків**.
...
Завантажте APK на телевізор файловим менеджером або поставте з комп'ютера:

```bash
adb install wystore.apk
```

При встановленні з телевізора Android один раз запитає дозвіл на встановлення невідомих застосунків.
...
- **З root** — без вікон Android ставиться все, включно з першим встановленням: встановлення йде
  через `pm` від root, повз діалог системного інсталятора. На пристроях із сервісами Google Play
  Protect може все одно перевірити файл, якого раніше не бачив. За замовчуванням root вимкнено.
...
Root не обов'язковий і за замовчуванням вимкнений. Щоб увімкнути: **Налаштування → Центр дозволів →
«Перевірити Root»**. Менеджер root (Magisk, KernelSU та інші) покаже свій запит — надайте доступ
Wy Store (`app.wystore`). Картка покаже «Доступний», при відмові — «Не надано». `su` шукається там,
де його тримає пристрій: у `PATH`, `/system/bin`, `/system/xbin`, `/sbin`, `/su/bin`,
`/debug_ramdisk`; root вважається наданим, лише якщо `id` повідомляє uid 0. Для KernelSU є окрема
інструкція (російською) — [KERNELSU_ROOT_RU.md](KERNELSU_ROOT_RU.md).

Перемикачі лежать у налаштуваннях, у групі «Фонові оновлення», і працюють, лише поки root
надано:

| Налаштування | Що робить |
|---|---|
| Тихе root-встановлення | Перші встановлення та оновлення, з RuStore і з GitHub, ідуть через `pm` від root без вікна Android. Wy Store записується інсталятором застосунку, а на Android 14 і новіших — і власником оновлень, так само як при встановленні через діалог. Застосунок числиться встановленим Wy Store і отримує оновлення як звичайно. |
| Тихе root-видалення | Вимкнено за замовчуванням. Кнопка «Видалити» видаляє застосунок через root одразу, без жодного підтвердження — разом з його даними і без запитання. Якщо в цей момент root не надано, відкривається звичайне вікно видалення Android. |
| Завантажувати оновлення у фоні (root) | Фонова перевірка за розкладом завантажує оновлення, навіть коли «Завантажувати оновлення одразу» вимкнено. |

З root фонова перевірка сама знаходить оновлення, завантажує і ставить його через root, поки
Wy Store закритий, без жодного натискання.

Якщо root відкликали, «Перевірити Root» покаже «Не надано», тихе встановлення вимкнеться, і
встановлення повернеться до вікна Android.

Коли викликається `su` — менеджер root може запитувати щоразу, якщо доступ надано не назавжди:

- за кнопкою «Перевірити Root»;
- при запуску, якщо увімкнено тихе встановлення або тихе видалення;
- у фоновій перевірці — лише коли «Завантажувати оновлення одразу» вимкнено, а «Завантажувати
  оновлення у фоні (root)» увімкнено;
- при встановленні та видаленні, якщо увімкнено відповідний перемикач.

Каталог, пошук і сторінки застосунків root не запитують ніколи.
...
- Root не обов'язковий. Без нього встановлення без підтвердження можливе лише для оновлень, лише
  на Android 12 і новіших і лише поки його не зупинить Play Protect. З root без вікон Android
  ставиться все, включно з першим встановленням, а видалення за бажанням проходить без
  підтвердження. Подробиці — у розділі «Як працюють оновлення».
- **Android TV і Google TV** на Android 9 і новіших — тим самим APK. Сповіщень на телевізорі немає,
  готове до встановлення показується на Головній.
